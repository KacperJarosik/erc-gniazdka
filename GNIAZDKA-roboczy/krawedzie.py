...
def main():
    ...
    if ids is not None:
        ...
        # kernel = np.ones((3, 3), np.uint8)  # Tworzymy 3x3 kernel do dylatacji
        # canny_51_dilated = cv2.dilate(canny_51, kernel, iterations=50)  # Pogrubienie krawędzi
        # canny_52_dilated = cv2.dilate(canny_52, kernel, iterations=50)  # Pogrubienie krawędzi
        # canny_63_64_dilated = cv2.dilate(canny_63_64, kernel, iterations=50)  # Pogrubienie krawędzi
        #
        # # # Przywrócenie do oryginalnego rozmiaru (erozja)
        # canny_51_dilated = cv2.erode(canny_51_dilated, kernel, iterations=50)
        # canny_52_dilated = cv2.erode(canny_52_dilated, kernel, iterations=50)
        # canny_63_64_dilated = cv2.erode(canny_63_64_dilated, kernel, iterations=50)
...
if __name__ == "__main__":
    ...
    if canny_63_64_dilated is not None:
        output_image = apply_mask_with_color(output_image, canny_63_64_dilated,
                                             (0, 0, 255))  # Czerwony dla maski 63 i 64
